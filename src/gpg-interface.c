...
static struct gpg_key_list_node *gpg_key_list_push(struct gpg_key_list *, struct _gpgme_key *);
static gpgme_error_t gpgme_passphrase_cb(void *, const char *, const char *, int, int);
static NORETURN void GPG_FATAL(const char *, gpgme_error_t);
...
void gpgme_context_init(struct gpgme_context **ctx, char *new_homedir)
{
	...
	if (new_homedir) {
		safe_create_dir(new_homedir, NULL, S_IRUSR | S_IWUSR | S_IXUSR);
	}

	err = gpgme_ctx_set_engine_info(*ctx, GPGME_PROTOCOL_OpenPGP, NULL, new_homedir);
	if (err)
		...
		struct strbuf *ciphertext, struct strbuf *output)
{
	...
	err = gpgme_op_decrypt(ctx, message_in, message_out);
	if (err)
...
int get_gpg_keys_from_keyring(struct gpgme_context *ctx,
		struct gpg_key_list *keys)
{
	gpgme_error_t err;
	int errsv = errno;
 ...
 * */
static NORETURN void GPG_FATAL(const char *msg, gpgme_error_t err)
{
