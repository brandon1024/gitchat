...
static int execute(struct child_process_def *cmd, int capture, struct strbuf *buffer)
{
    ...
    if(cmd->git_cmd) {
        ...
        command = (char *)calloc(strlen(git_cmd) + strlen(cmd_args) + 1, sizeof(char));
        if(command == NULL) {
    ...
    if(fp == NULL) {
        fprintf(stderr, "fatal error: unable to create pipe to shell process. %x: %s\n", errno, strerror(errno));
        return 1;
    }
    ...
    if(status == -1) {
        fprintf(stderr, "fatal error: unable to close pipe to shell process. %x: %s\n", errno, strerror(errno));
        return 1;
    }
