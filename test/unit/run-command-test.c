...
TEST_DEFINE(run_command_provisioned_in)
{
	...
	TEST_START() {
		...
		for (int i = 0; i < 4096; i++) {
			...
			if (write(cmd.in_fd[1], buffer, expected) != expected) {
				write_failed = 1;
				break;
			}
		}
...
TEST_DEFINE(run_command_provisioned_out)
{
	...
	TEST_START() {
		...
		for (int i = 0; i < 4096; i++) {
			...
			ssize_t bytes_read = read(cmd.out_fd[0], capture_buffer, 1024);
			if (bytes_read < 0) {
				read_failed = 1;
				break;
			}
...
TEST_DEFINE(run_command_provisioned_streams)
{
	TEST_START() {
...
int run_command_test(struct test_runner_instance *instance)
{
	struct unit_test tests[] = {
			...
			{ "Executing a child process with a provisioned streams should read and write appropriately", run_command_provisioned_streams },
			{ NULL, NULL }
	};
